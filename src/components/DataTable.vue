        ...
        <template #content="slotProps">
          ...
          >
            ...
            <tbody
              ...
              :style="{
                height: `calc(${slotProps.spacerStyle.height} - ${
                  slotProps.rows.length * slotProps.itemSize
                }px)`,
              }"
              class="p-datatable-virtualscroller-spacer"
            ></tbody>
            <DTTableFooter
              :columnGroup="footerColumnGroup"
              :columns="slotProps.columns"
            />
          </table>
        </template>
...
import {
  ...
  FilterOperator,
  FilterService,
} from "../../node_modules/primevue/api";
...
export default {
  ...
  methods: {
    ...
    onSpaceKey(event, rowData, rowIndex, slotProps) {
      ...
      if (event.shiftKey && this.selection !== null) {
        ...
        if (this.selection.length > 0) {
          ...
          firstSelectedRowIndex = ObjectUtils.findIndexInList(
            this.selection[0],
            data
          );
          lastSelectedRowIndex = ObjectUtils.findIndexInList(
    ...
    findFirstSelectableRow() {
      const firstRow = DomHandler.findSingle(
        this.$refs.table,
        ".p-selectable-row"
      );
    ...
    exportCSV(options, data) {
      ...
      for (let i = 0; i < this.columns.length; i++) {
        ...
        if (
          this.columnProp(column, "exportable") !== false &&
          this.columnProp(column, "exportFooter")
        ) {
    ...
    resizeTableCells(newColumnWidth, nextColumnWidth) {
      ...
      let innerHTML = "";
      let selector = `.p-datatable[${
        this.attributeSelector
      }] > .p-datatable-wrapper ${
        this.virtualScrollerDisabled ? "" : "> .p-virtualscroller"
      } > .p-datatable-table`;
      ...
      widths.forEach((width, index) => {
        ...
        innerHTML += `
                    ${selector} > .p-datatable-thead > tr > th:nth-child(${
          index + 1
        }),
                    ${selector} > .p-datatable-tbody > tr > td:nth-child(${
          index + 1
        }),
                    ${selector} > .p-datatable-tfoot > tr > td:nth-child(${
          index + 1
        }) {
                        ${style}
                    }
    ...
    isRowGroupExpanded(rowData) {
      if (this.expandableRowGroups && this.expandedRowGroups) {
        let groupFieldValue = ObjectUtils.resolveFieldData(
          rowData,
          this.groupRowsBy
        );
    ...
    createResponsiveStyle() {
      if (!this.responsiveStyleElement) {
        ...
        let tableSelector = `.p-datatable-wrapper ${
          this.virtualScrollerDisabled ? "" : "> .p-virtualscroller"
        } > .p-datatable-table`;
