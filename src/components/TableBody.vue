      ...
      >
        <tr
          v-if="
            ...
            shouldRenderRowGroupHeader(value, rowData, getRowIndex(index))
          "
        ...
        >
          ...
          >
            <DTBodyCell
              ...
              :rowTogglerIcon="
                columnProp(col, 'expander') ? rowTogglerIcon(rowData) : null
              "
              :frozenRow="frozenRow"
              :rowspan="
                rowGroupMode === 'rowspan'
                  ? calculateRowGroupSize(value, col, getRowIndex(index))
                  : null
              "
              :editMode="editMode"
        ...
        <tr
          v-if="
            templates['expansion'] && expandedRows && isRowExpanded(rowData)
          "
        ...
        <tr
          v-if="
            ...
            rowGroupMode === 'subheader' &&
            shouldRenderRowGroupFooter(value, rowData, getRowIndex(index))
          "
...
export default {
  ...
  methods: {
    ...
    rowTogglerIcon(rowData) {
      const icon = this.isRowExpanded(rowData)
    ...
    rowGroupTogglerIcon(rowData) {
      const icon = this.isRowGroupExpanded(rowData)
        ? this.expandedRowIcon
        : this.collapsedRowIcon;
    ...
    updateFrozenRowStickyPosition() {
      this.$el.style.top =
        DomHandler.getOuterHeight(this.$el.previousElementSibling) + "px";
    },
...
.dis {
  ...
  background-color: rgba(156, 161, 168, 0.1) !important;
}
</style>
